...
def get_candidates_for_query_vectorised(predicted_buckets, model_indexes, model_offsets, data, freq_threshold):
    """
    For a single query:
    ...
    candidate_data_slices = [
        data[model_indexes[model][start : end]]
        for model in range(r)
        for start, end in zip(start_indices[model], end_indices[model])
    ]
    candidate_indices_slices = [
    ...
    if candidate_data_slices:
        candidate_data = np.concatenate(candidate_data_slices)
        candidate_indices = np.concatenate(candidate_indices_slices)
    else:
    ...
    counts = Counter(candidate_indices)
    # Get valid elements (those whose counts are greater than or equal to threshold)
    valid_indices = {idx for idx, count in counts.items() if count >= freq_threshold}
    mask = np.isin(candidate_indices, list(valid_indices))
    # filtered_indices = candidate_indices[mask]
    # filtered_data = candidate_data[mask]
    unique_valid_indices = np.unique(candidate_indices[mask])
    first_occurrences = np.array([np.where(candidate_indices == idx)[0][0] for idx in unique_valid_indices])
    unique_filtered_data = candidate_data[first_occurrences]

    return unique_valid_indices, unique_filtered_data
...
def process_query_batch(data, neighbours, query_vectors, candidate_buckets, indexes, offsets, freq_threshold, requested_amount, batch_process_start):
    ...
    for i, query in enumerate(query_vectors):
        ...
        candidate_indices, candidate_data = get_candidates_for_query_vectorised(predicted_buckets, indexes, offsets, data, freq_threshold)
        ...
        if len(candidate_indices) <= requested_amount:
            query_end = time.time()
            batch_results[i] = (candidate_indices, neighbours[i], 0, (query_end-query_start) + base_time_per_query), ut.recall_single(candidate_indices, neighbours[i])
        else:
            reordering_start = time.time()
            final_neighbours, dist_comps, true_nns_time, fetch_data_time, current_mem = reorder(data, query, candidate_indices, candidate_data, requested_amount)
            memory = current_mem if current_mem > memory else memory
            del candidate_indices, candidate_data
            query_end = time.time()
...
def reorder(data, query_vector, candidate_indices, candidate_data, requested_amount):
    '''
    ...
    final_neighbours = candidate_indices[neighbours]
