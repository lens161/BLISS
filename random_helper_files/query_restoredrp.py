...
import time
from sklearn.random_projection import SparseRandomProjection, _sparse_random_matrix
...
def load_data_for_inference(dataset: ds.Dataset, config: Config, SIZE, DIM):
    ...
    return data, test[:100], neighbours[:100], using_memmap
...
def reorder_approx(candidate_data, query_vector, candidates, requested_amount, process, using_memmap, track_mem = False):
    '''
    Do true distance calculations on the candidate set of vectors and return the top 'requested_amount' candidates.
    ''' 
    #TODO: check if it makes sense to sort candidates before fetching from memmap to improve access pattern
...
def process_query_batch_twostep(data, neighbours, query_vectors, candidate_buckets, indexes, offsets, rp_files, freq_threshold, requested_amount, m, candidate_amount_limit, batch_process_start, process, max_cand_size, mem_tracking, projection_matrix, rp_dim, timers, using_memmap):
    ## input: candidate_buckets np array for a batch of queries
    ## output: the ANNs for the batch of queries, dist_comps per query, recall per query
    batch_results = [[] for i in range(len(query_vectors))]
    batch_process_end = time.time()
    ...
    for i, query in enumerate(query_vectors):
        ...
        if cand_size <= requested_amount:
            ...
            rp_dist_comps = cand_size
            if candidate_rp_data is not None:
                approx_candidates = get_pseudo_inverse(candidate_rp_data, projection_matrix)
                final_neighbours, dist_comps, current_mem = reorder_approx(approx_candidates, query, candidate_ids, requested_amount, process, using_memmap, track_mem)
            else:
...
def query_multiple_batched_twostep(data, index, vectors, neighbours, config: Config, using_memmap):
    ...
    query_loader = DataLoader(queries_batched, batch_size=config.query_batch_size, shuffle=False, num_workers=8)
    # transformer = SparseRandomProjection(n_components=config.rp_dim, random_state=config.rp_seed)
    projection_matrix = get_sparse_projection_matrix(config.rp_dim, data.shape[1], random_state=config.rp_seed)
...
def get_sparse_projection_matrix(n_components, original_dim, density='auto', random_state=None):
    # This creates the same random matrix as SparseRandomProjection would
    return _sparse_random_matrix(n_components, original_dim, density=density, random_state=random_state)
...
def get_pseudo_inverse(rp_vectors, projection_matrix):
    if issparse(projection_matrix):
        projection_matrix = projection_matrix.toarray()
    pseudo_inverse_matrix = np.linalg.pinv(projection_matrix.T)
    approx_vectors = rp_vectors.dot(pseudo_inverse_matrix)
