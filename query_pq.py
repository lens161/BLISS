...
def load_data_for_inference(dataset, config: Config, SIZE, DIM):
    ...
    data = np.memmap(memmap_path, mode='r', shape=(SIZE, 8), dtype=np.float32) if SIZE >10_000_000 else np.memmap(memmap_path,shape=(SIZE, 8), mode='r', dtype=np.float32)[:]
    sample_size = SIZE if SIZE < 2_000_000 else 1_000_000

    index_pq = None
    with open(f"data/{config.dataset_name}_sample{sample_size}_pq.pkl", 'rb') as f:
        (index_pq, m) = pickle.load(f)
    ...
    return data, test, neighbours, index_pq, m
